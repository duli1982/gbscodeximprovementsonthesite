(() => {
    ...
    function initializeData(focusPointsData, promptsData) {
        const processedFocusPoints = focusPointsData.map((point, index) => ({
            ...point,
            id: `fp-${index}`,
            type: 'Focus Point'
        }));

        focusPoints = processedFocusPoints;

        const processedPrompts = [];
        for (const category in promptsData) {
            for (const subcategory in promptsData[category]) {
                promptsData[category][subcategory].forEach(prompt => {
                    processedPrompts.push({
                        ...
                        actions: [prompt.content]
                    });
                });
            }
        }

        allContent = [...processedFocusPoints, ...processedPrompts];

        categoryCounts = allContent.reduce((acc, point) => {
            acc[point.category] = (acc[point.category] || 0) + 1;
            return acc;
        }, {});
        categoryCounts['All'] = allContent.length;

        const savedProgress = localStorage.getItem('sourcingFocusProgress');
        if (savedProgress) {
            progressState = JSON.parse(savedProgress);
        } else {
            allContent.forEach(point => {
                if (point.type === 'Focus Point') {
    ...
    function createCard(point) {
        ...
        let actionsHtml;
        let progressHtml = '';
        let headerHtml = `<span class="text-sm font-semibold text-indigo-600">${point.category}</span>`;

        if (point.type === 'Focus Point') {
            actionsHtml = point.actions.map((action, index) => {
            ...
            actionsHtml = `<li class="p-3 text-gray-700 whitespace-pre-wrap bg-gray-50 rounded-lg">${point.actions[0]}</li>`;
            headerHtml = `<span class="text-sm font-semibold text-purple-600">${point.type}</span>`;
        }
        ...
        cardElement.innerHTML = `
            ...
            <div>
                <p class="font-semibold mb-2 text-gray-700">${point.type === 'Focus Point' ? 'Action Items:' : 'Content:'}</p>
    ...
    function loadDeck(category = 'All') {
        currentDeck = (category === 'All') ? [...allContent] : allContent.filter(p => p.category === category);
        currentIndex = 0;
    ...
    function populateFilters() {
        const categories = ['All', ...new Set(allContent.map(p => p.category))];
    ...
    function handleDirectLink() {
        ...
        const promptId = urlParams.get('showPrompt');
        const targetId = focusId || promptId;

        if (targetId) {
            const itemToShow = allContent.find(item => item.id === targetId);
            if (itemToShow) {
                document.getElementById('card-of-the-day-section').style.display = 'none';
                document.querySelector('.text-center.my-8').style.display = 'none';

                currentDeck = [itemToShow];
                currentIndex = 0;
                showCardInLibrary(0);
    ...
    document.addEventListener('DOMContentLoaded', () => {
        Promise.all([
            fetch('focus.json').then(res => res.json()),
            fetch('prompts.json').then(res => res.json())
        ])
        .then(([focusData, promptsData]) => {
            initializeData(focusData, promptsData.promptData);
            renderCardOfTheDay();
            populateFilters();
            loadDeck('All');
            handleDirectLink();
